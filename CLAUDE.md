...
- **WalletReportController** - `GET /api/wallet-report` paginated per-wallet performance report using a single SQL query with LEFT JOIN subqueries for aggregation, sorting (ORDER BY), and pagination (LIMIT/OFFSET). WalletScoring is computed only for the current page's wallets (not all). `GET /api/wallet-report/summary` returns aggregate totals (profitable/losing/paused counts, best performer, average score) across all wallets — fetched separately from table data. `GET /api/category-report` aggregates performance by market category (Crypto, Politics, Sports, etc.) — maps each market's event tags (from cache) to a primary category using config `market_category_tags`, then groups trade_history and positions by category. Returns P&L, win rate, trade counts, invested, and enabled/disabled status per category. Uses only cached event tags (no API calls) — uncached slugs go to "Other".
...
| Model          | Purpose                                                        |
|----------------|----------------------------------------------------------------|
| `Position`     | Open positions (asset_id, market_slug, market_question, market_image, outcome, shares, buy_price, current_price, tp_price, sl_price) |
...
| `POLYMARKET_MAX_GLOBAL_MARKET_USDC` | 30                        | Max total exposure per market across all wallets |
...
| `POLYMARKET_MOMENTUM_FILTER`  | true                            | Skip BUY if midpoint < trade price   |
...
| `POLYMARKET_ENABLE_TP_SL`     | true                            | Enable take-profit/stop-loss auto-exits |
| `POLYMARKET_TP_PERCENTAGE`    | 20                              | Take-profit threshold (% above entry) |
| `POLYMARKET_SL_PERCENTAGE`    | 15                              | Stop-loss threshold (% below entry)   |
| `POLYMARKET_MAX_POSITION_AGE_HOURS` | 72                       | Auto-close positions older than N hours |
| `POLYMARKET_MAX_MARKET_DURATION_DAYS` | 30                     | Skip BUY for markets ending > N days away |
...
| `POLYMARKET_USE_KELLY_SIZING` | false                           | Enable Kelly Criterion position sizing        |
| `POLYMARKET_KELLY_FRACTION_MULTIPLIER` | 0.5                    | Kelly fraction multiplier (0.5 = half Kelly)  |
| `POLYMARKET_KELLY_MIN_TRADES` | 20                              | Min closed trades before Kelly kicks in       |
...
| `POLYMARKET_CATEGORY_POP_CULTURE` | true                         | Enable/disable Pop Culture market trades      |
| `POLYMARKET_CATEGORY_BUSINESS`  | true                           | Enable/disable Business & Finance trades      |
| `POLYMARKET_CATEGORY_SCIENCE`   | true                           | Enable/disable Science & Weather trades       |
...
2. **Every 30s** - `bot:update-prices`: Fetches midpoints for all open positions concurrently, updates DB. Falls back to resolution check if midpoint fails. **After price updates, checks TP/SL thresholds**: if `current_price >= tp_price` → auto-close (take-profit); if `current_price <= sl_price` → auto-close (stop-loss) via `TradeCopier::closePosition()`. Backfills market metadata (slug, question, image, outcome) for up to 5 positions per cycle that are missing any field (via Gamma API `getMarketMetadata()`, cached 24h). Also caches Polymarket USDC balance in BotMeta (skipped in dry-run)
...
7. **Market duration filter** (BUY only): skip if market end date is more than `max_market_duration_days` (default 30) days away — avoids capital being locked in long-duration markets
8. **Momentum confirmation filter** (BUY only): skip if current midpoint < tracked trader's execution price (price has already dropped since they bought)
9. Calculate size: dynamic amount based on wallet score and available balance (Kelly Criterion if enabled, else hybrid % with min/max caps), all held shares for sells
10. Price sanity: skip if midpoint deviates > 3 cents from trade price
11. Trading balance limit: skip if trade amount exceeds available capital (limit - invested + realized P&L)
12. Per-market exposure cap: skip if would exceed $10 per market
13. **Per-wallet exposure cap**: skip if total invested from this wallet would exceed $20
14. **Global per-market exposure cap**: skip if total invested in this market_slug across all wallets would exceed $30
15. Place order via CLOB API (or log in dry-run mode)
16. If matched/dry-run: extract actual fill price, update position immediately with weighted-average buy price, fetch market_slug
17. If live/delayed: create PendingOrder record, defer position update. `bot:check-orders` will poll for fill and apply update or auto-cancel after 10min TTL
...
- **Dynamic position sizing** - Hybrid % of available balance with min/max caps, scaled by wallet composite score. Three tiers: score 70+ (0.5%, max $10), score 50-69 (0.3%, max $5), score 30-49 (0.15%, max $3). All tiers have a $1 floor. Wallets with no score or score <30 use the fixed fallback amount ($2). As available balance grows, trade sizes grow proportionally; as it shrinks, sizes shrink automatically. When `fixed_amount_override` is set (via Settings tab), all dynamic sizing is bypassed and every BUY trade uses that fixed USDC amount. **Kelly Criterion sizing** (optional, disabled by default): when `use_kelly_sizing` is enabled, `computeTradeAmount()` uses the mathematically optimal Kelly fraction instead of tier percentages. Kelly formula: `f* = p - (1-p)/b` where `p` = win rate, `b` = avg_win/avg_loss. Default uses half-Kelly (`kelly_fraction_multiplier=0.5`) for safety — proven to have ~75% of full Kelly's growth rate with massively reduced variance. Falls back to tier-based sizing when a wallet has fewer than `kelly_min_trades` (default 20) closed trades. Priority: `fixed_amount_override` > Kelly > tier-based.
- **Global per-market exposure cap** - `max_global_market_usdc` (default $30) limits total invested in any single `market_slug` across ALL tracked wallets. Prevents concentration risk when multiple wallets buy the same popular market. Queries by `market_slug` (not `asset_id`) so both Yes/No tokens of the same market are counted together. Includes pending BUY orders with populated `market_slug`. Skipped when metadata is unavailable (don't block unknown markets).
...
- **Market category filtering** - BUY trades can be filtered by market category (Crypto, Politics, Sports, Pop Culture, Business, Science). Categories are toggled on/off from the Settings > Market Categories section. Tags are fetched from the Gamma API `/events` endpoint via `getEventTags()` (cached 24h) and included in `getMarketMetadata()`. A config mapping (`market_category_tags`) maps each category to its associated tag slugs (e.g., 'crypto' → ['crypto', 'airdrops']). When a category is disabled, `TradeCopier::isCategoryBlocked()` checks if the market's tags match any disabled category. Unknown/untagged markets are always allowed through. SELL trades are never category-filtered.
- **Momentum confirmation filter** - BUY-only filter (enabled by default via `momentum_filter`). When a BUY trade is detected, the current midpoint is fetched and compared to the tracked trader's execution price. If `midpoint < trade_price`, the price has already dropped since the trader bought — negative momentum — and the trade is skipped. This is different from price tolerance (which allows deviation in both directions); momentum only allows same or higher price. The midpoint fetch is shared with the price tolerance check to avoid duplicate API calls.
- **Take-profit / Stop-loss exits** - Independent auto-exit system (enabled by default via `enable_tp_sl`). When a BUY fill is applied (`applyBuyFill()`), TP and SL prices are calculated from the weighted-average buy price: `tp_price = buy_price * (1 + tp_percentage/100)`, `sl_price = buy_price * (1 - sl_percentage/100)`. Default thresholds: +20% TP, -15% SL. These are stored on the Position model and recalculated on every additional buy (as buy_price changes with weighted averaging). Every 30s, `bot:update-prices` checks all positions after updating prices: if `current_price >= tp_price`, the position is auto-closed via `closePosition()` (take-profit); if `current_price <= sl_price`, it's auto-closed (stop-loss). Both produce log entries (`tp_exit_triggered` / `sl_exit_triggered`). TP/SL prices are cleared when a position is fully closed.
- **Max position age** - Positions older than `max_position_age_hours` (default 72h / 3 days) are auto-closed by `bot:update-prices` at current market price. Uses `opened_at` timestamp on Position model. Prevents capital from being stuck in long-running trades. Set to 0 to disable.
- **Market duration filter** - BUY-only filter that skips markets whose `endDate` (from Gamma API) is more than `max_market_duration_days` (default 30) days away. Prevents entering positions in markets that won't resolve for months or years. The `endDate` is included in `getMarketMetadata()` return value (cached 24h). Markets without an end date or with past end dates are allowed through.
