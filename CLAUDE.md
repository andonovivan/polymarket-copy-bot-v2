...
7. **Momentum confirmation filter** (BUY only): skip if current midpoint < tracked trader's execution price (price has already dropped since they bought)
8. Calculate size: dynamic amount based on wallet score and available balance (Kelly Criterion if enabled, else hybrid % with min/max caps), all held shares for sells
9. Price sanity: skip if midpoint deviates > 3 cents from trade price
...
10. **Global per-market exposure cap**: skip if total invested in this market_slug across all wallets would exceed $30
11. Place order via CLOB API (or log in dry-run mode)
